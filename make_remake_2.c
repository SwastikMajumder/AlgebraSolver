...
#define MAX_TT_SIZE 4000

#define IS_CHAR_SMALL_LETTER(x) (((x)>='a')&&((x)<='z'))
#define IS_CHAR_CAP_LETTER(x) (((x)>='A')&&((x)<='Z'))
#define IS_CHAR_NUMBER(x) (((x)>='0')&&((x)<='9'))
#define IS_CHAR_CALCULUS(x) ((x)=='m'||(x)=='n')

#define MAX(x, y) (((x) > (y)) ? (x) : (y))
#define MIN(x, y) (((x) < (y)) ? (x) : (y))

char *FUNCTION_FULL_NAME_LIST[FUNCTION_COUNT] = {"add", "mul", "pow", "div", "sin", "cos", "tan", "csc", "sec", "cot", "sqt", "lwn", "exp", "int"};
...
int apply_formula(struct EQUATION *equation, struct EQUATION *formula, struct EQUATION *variable_list[26]){
	...
	if (formula->FUNCTION_TYPE == TYPE_VARIABLE){ // if its a variable 
		...
		if (IS_CHAR_CALCULUS(formula->VARIABLE_NAME)){
			if (equation->FUNCTION_TYPE == TYPE_VARIABLE && IS_CHAR_CALCULUS(equation->VARIABLE_NAME)){
				if (variable_list[formula->VARIABLE_NAME-'a'] == 0){
					variable_list[formula->VARIABLE_NAME-'a'] = malloc(sizeof(struct EQUATION));
					duplicate_equation(variable_list[formula->VARIABLE_NAME-'a'], equation);
				} else if (variable_list[formula->VARIABLE_NAME-'a']->FUNCTION_TYPE == TYPE_VARIABLE &&
						   variable_list[formula->VARIABLE_NAME-'a']->VARIABLE_NAME == equation->VARIABLE_NAME){
					return 1;
				} else return 0;
			} else {
				return 0;
		...
		if (formula->VARIABLE_NAME == 'k' && is_contain_m(equation)) return 0;
...
int is_contain_m(struct EQUATION *equation){
	if (equation->FUNCTION_TYPE == TYPE_VARIABLE && IS_CHAR_CALCULUS(equation->VARIABLE_NAME )) return 1;
	if (equation->FUNCTION_TYPE == TYPE_VARIABLE || equation->FUNCTION_TYPE == TYPE_CONSTANT) return 0;
	if (equation->FUNCTION_TYPE >= SINE) equation->NUMBER_OF_CHILDREN = 1;
	int i;
	for (i=0; i<equation->NUMBER_OF_CHILDREN; ++i){
		if (is_contain_m(equation->CHILDREN[i])) return 1;
	}
	return 0;
}
...
void replace_instances(struct EQUATION *equation, struct EQUATION *instance_find, struct EQUATION *instance_replace){
	if (equation->FUNCTION_TYPE == TYPE_VARIABLE || equation->FUNCTION_TYPE == TYPE_CONSTANT) return;
	if (compare_equation(equation, instance_find)){
		struct EQUATION *tmp= malloc(sizeof(struct EQUATION));
		duplicate_equation(tmp, instance_replace);
		equation = tmp;
	}
	return;
	int i;
	for (i=0; i<equation->NUMBER_OF_CHILDREN; ++i){
		replace_instances(equation->CHILDREN[i], instance_find, instance_replace);
	}
}

void possible_equation_integration_substitute(struct EQUATION *equation, struct EQUATION *attempt_list[], int *attempt_list_count){
	if (is_contain_m(equation)){
		struct EQUATION *tmp= malloc(sizeof(struct EQUATION));
		duplicate_equation(tmp, equation);
		attempt_list[(*attempt_list_count)++]= tmp;
	}
	if (equation->FUNCTION_TYPE == TYPE_VARIABLE ||equation->FUNCTION_TYPE == TYPE_CONSTANT) return;
	int i;
	for (i=0;i<equation->NUMBER_OF_CHILDREN; ++i){
		possible_equation_integration_substitute(equation->CHILDREN[i], attempt_list, attempt_list_count);
	}
}
...
void generate_equation(struct EQUATION *equation, char *formula_input_list[], char *formula_output_list[], int formula_count, struct EQUATION *equation_list[], int *count){
	...
	for (i=0; i<formula_count; ++i){ // for all formulas
		...
		//printf("dry %s\n", formula_input_list[i]);
		//print_equation(&curr_formula_input);
		struct EQUATION *curr = malloc(sizeof(struct EQUATION));
	...
	for (j=1;;++j){
		...
		if (compare_equation(curr, equation) == 0){//printf("dry");
			equation_list[*count] = malloc(sizeof(struct EQUATION));
			duplicate_equation(equation_list[*count], curr);
			++(*count);
		}
...
int formula_function(char *formula_input_list[], char *formula_output_list[]){
	...
	char *fl = "int(sin(m))=-cos(m), a+0=a, ab+abc=ab(c+1), 0*a=0, int(k)=km, int(m)=m^2/2, int(a+b)=int(a)+int(b), int(ka)=kint(a), a+a=2a, int(m^a)=(m^(a+1))*((a+1)^-1), a^1=a, a^b*a^c=a^(b+c), (ab)^c=a^c*b^c, a^2=aa, aa^(-1)=1, 1-a^2=(1+a)(1-a), sin(a)^2=1-cos(a)^2, ab+ac=a(b+c), aa=a^2, a+cba=a(bc+1), ab+ac=a(b+c), (a+b)(c+d)=ac+ad+bc+bd, a(b+c)=ab+ac, sin(a)^2+cos(a)^2=1, (a-b)^2=a^2-2ab+b^2, csc(a)=1/sin(a), cot(a)=cos(a)/sin(a)";
	char flo[500];
	int index=0;
	for (i=0; fl[i]; ++i){
		if (fl[i]==' ') continue;
...
void make_list(char *f_input[], char *f_output[], int f_count, struct EQUATION *equation, int *equation_list_count, struct EQUATION *equation_list[]){
	...
	for (i=0; i<(*equation_list_count); ++i){
		//print_equation(equation);
		//print_equation(equation_list[i]);
		//printf("-----------\n");
		++counting;
...
void search(char *f_input[], char *f_output[], int f_count, struct MATH_STEP *output, struct EQUATION *eq, struct EQUATION *target){
	...
	while (queue_end){
		...
		//printf("line break\n");
		//print_equation(move_list[0]);
		//print_equation(move_list[1]);
		//return;
...
int main (){
	...
	string_to_equation_short(&start, "int(m+m)");
	print_equation(&start);
	struct EQUATION target;
	string_to_equation_short(&target, "m^2");
